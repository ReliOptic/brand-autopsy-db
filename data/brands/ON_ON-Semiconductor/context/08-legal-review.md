# 08. Legal Review — ON Semiconductor (ON)
...
|----------|----------|-------------|
| 10-K — Part I, Item 1A (Risk Factors) | Legal & regulatory risks | [SEC EDGAR ON](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&company=ON+Semiconductor&CIK=&type=10-K&dateb=&owner=include&count=10&search_text=&action=getcompany) |
| 10-K — Part I, Item 3 (Legal Proceedings) | Active litigation disclosures | Same 10-K, Item 3 |
| 8-K Filings | Material legal events | [SEC EDGAR ON 8-K](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&company=ON+Semiconductor&CIK=&type=8-K&dateb=&owner=include&count=20&search_text=&action=getcompany) |
...
ON Semiconductor maintains a trademark portfolio protecting its brand names, product names, and logos in relevant commercial classes. Trademark registrations are publicly searchable via the USPTO TESS database and equivalent international registries. (T1_OFFICIAL, USPTO public records)
...
> All descriptions below are factual summaries of publicly disclosed legal proceedings as referenced in ON Semiconductor's SEC filings.

ON Semiconductor's active litigation and legal proceedings are disclosed in the company's 10-K, Part I, Item 3 (Legal Proceedings) and in the Notes to Financial Statements under Commitments and Contingencies. (SEC 10-K)
...
ON Semiconductor's regulatory environment reflects the requirements applicable to semiconductors within information technology. Key regulatory areas as typically disclosed in the 10-K include:
...
**Assessment**: Based on publicly available records, ON Semiconductor's litigation posture reflects standard commercial practices for a company of its size in the information technology sector. (T4_INFERRED)
...
### 5.4 Writing Risk Guide — ON Semiconductor
...
ON Semiconductor's ESG disclosures are available in the company's sustainability report, proxy statement (DEF 14A), and 10-K filing. Key areas include:
...
Refer to ON Semiconductor's 10-K Item 1A for company-specific risk factor disclosure.
```
...
**Overall Project Risk Level for ON Semiconductor Content: MEDIUM**
